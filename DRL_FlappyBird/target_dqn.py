...
def train(model, q_target, start, beginning_episode = 0, beginning_iteration = 0):
    ...
    while iteration < model.number_of_iterations:
        ...
        max_q_prime = q_target(state_1_batch)

        # set y_j to r_j for terminal state, otherwise to r_j + gamma*max(Q)
        y_batch = torch.cat(tuple(reward_batch[i] if minibatch[i][4]
                                  else reward_batch[i] + model.gamma * torch.max(max_q_prime[i])
        ...
        Q_list.append(np.max(output.cpu().detach().numpy()))

        if iteration % args.save_every == 0:
            torch.save(model, args.model_save_path + "/policy_model_" + str(iteration) + ".pth")
            ...
            torch.save(q_target, args.target_save_path + "/targetq_model" + str(iteration) + ".pth")
            

        if iteration % args.update_target_every == 0:
...
def config_parser():
    ...
    parser.add_argument("--target_save_path", type=str, 
                        help='path to save target weights')
    ...
    parser.add_argument("--update_target_every", type=int, default=50000, 
                        help='frequency to update target')
...
def main(mode):
    ...
    if mode == 'test':
        ...
        #making target_network (a clone)
        qtarget = NeuralNetwork()

        if cuda_is_available:  # put on GPU if CUDA is available
            qtarget = qtarget.cuda()
        qtarget.load_state_dict(model.state_dict())

        start = time.time()

        train(model,qtarget,start,args.beginning_episode,args.beginning_iteration)
