...
def softMM(x, mm_omega, alpha = 10, normalize = True):
    if normalize == True:
        b = torch.max(mm_omega * x)
        b2 = torch.max(alpha * x)
    else:
        b = 0
        b2 = 0
    
    denom = torch.sum(torch.exp((alpha * x) - b2))
    numerator = torch.exp((alpha * x) - b2)
    expSum = torch.exp(mm_omega * x - b)
    soft = numerator/denom
    total = soft * expSum
    mm = b + torch.log(total.sum())
    mm = mm / mm_omega

    return mm

def train(model, start, alpha, temperature = 1,beginning_episode = 0, beginning_iteration = 0):
    ...
    while iteration < model.number_of_iterations:
        ...
        y_batch = torch.cat(tuple(reward_batch[i] if minibatch[i][4]
                                  else reward_batch[i] + model.gamma * softMM(output_1_batch_without_grad[i],temperature, alpha)
        ...
        Q_list.append(softMM(out_put_without_grad,temperature, alpha).item())

        if iteration % args.save_every == 0:
            torch.save(model, args.model_save_path + "/softmm_model_" + str(iteration) + ".pth")
            np.savetxt(args.results_save_path + "/reward_loss_" + str(iteration) + ".csv", np.array(reward_list))
            np.savetxt(args.results_save_path + "/softmm_Q_" + str(iteration) + ".csv", np.array(Q_list))
        ...
        print("episode:", episode, "iteration:", iteration, "elapsed time:", time.time() - start, "epsilon:", epsilon, "action:",
              ...
              softMM(out_put_without_grad,temperature,alpha).item())
...
def main(mode):
    ...
    if mode == 'test':
        ...
        train(model, start,alpha = args.alpha, temperature = args.temperature, beginning_episode = args.beginning_episode, beginning_iteration = args.beginning_iteration)
